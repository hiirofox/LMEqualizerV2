...
juce::AudioProcessorValueTreeState::ParameterLayout LModelAudioProcessor::createParameterLayout()
{
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>("pow", "pow", 0, 1, 1));
	layout.add(std::make_unique<juce::AudioParameterFloat>("lpm", "lpm", 0, 1, 1));
...
void LModelAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	float lpm = *Params.getRawParameterValue("lpm");
	...
	lpm = (expf((lpm - 1.0) * 8.0) - expf(-8.0)) / (1.0 - expf(-8.0));
	pw *= 5.0;

	stftl.SetDelay(lt, rt, 0, fb, lpm, pw, dry);
	stftr.SetDelay(lt, rt, 0, fb, lpm, pw, dry);
...
void LModelAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
	...
	auto envelopeXml = enveFunc1.saveToXml();
	if (envelopeXml) {
...
void LModelAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
	...
	if (envelopeXml) {
		bool success = enveFunc1.loadFromXml(envelopeXml);
		if (!success) {
			DBG("Warning: Failed to load envelope data, using default");
			enveFunc1.resetToDefault();
		}
	}
	else {
		// 如果没有找到包络数据，重置为默认值
